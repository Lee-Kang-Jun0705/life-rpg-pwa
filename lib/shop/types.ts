...
function getItemIcon(item: Item): string {
  ...
  if (item.type === 'weapon') {
    ...
    if (name.includes('도끼') || name.includes('axe')) {
      return ITEM_ICONS.axe
    }
    if (name.includes('창') || name.includes('spear')) {
      return ITEM_ICONS.spear
    }
    if (name.includes('활') || name.includes('bow')) {
      return ITEM_ICONS.bow
    }
    if (name.includes('지팡이') || name.includes('staff')) {
      return ITEM_ICONS.staff
    }
    if (name.includes('망치') || name.includes('hammer')) {
      return ITEM_ICONS.hammer
    }
    if (name.includes('단검') || name.includes('dagger')) {
      return ITEM_ICONS.dagger
    }
  ...
  // 방어구
  if (item.type === 'armor') {
    if (name.includes('투구') || name.includes('모자') || name.includes('helm')) {
      return ITEM_ICONS.helmet
    }
    if (name.includes('부츠') || name.includes('신발') || name.includes('boot')) {
      return ITEM_ICONS.boots
    }
    if (name.includes('장갑') || name.includes('glove')) {
      return ITEM_ICONS.gloves
    }
    if (name.includes('방패') || name.includes('shield')) {
      return ITEM_ICONS.shield
    }
  ...
  if (item.type === 'accessory') {
    ...
    if (name.includes('목걸이') || name.includes('necklace')) {
      return ITEM_ICONS.necklace
    }
    if (name.includes('귀걸이') || name.includes('earring')) {
      return ITEM_ICONS.earring
    }
    if (name.includes('팔찌') || name.includes('bracelet')) {
      return ITEM_ICONS.bracelet
    }
    return ITEM_ICONS.ring
  }
  ...
  if (item.type === 'consumable') {
    ...
    if (name.includes('빵') || name.includes('고기') || name.includes('음식')) {
      return ITEM_ICONS.food
    }
    if (name.includes('주문서') || name.includes('scroll')) {
      return ITEM_ICONS.scroll
    }
    if (name.includes('폭탄') || name.includes('bomb')) {
      return ITEM_ICONS.bomb
    }
    return ITEM_ICONS.potion
  }
  ...
  if (item.type === 'material') {
    ...
    if (name.includes('보석') || name.includes('다이아') || name.includes('루비')) {
      return ITEM_ICONS.gem
    }
    if (name.includes('가죽') || name.includes('leather')) {
      return ITEM_ICONS.leather
    }
    if (name.includes('나무') || name.includes('wood')) {
      return ITEM_ICONS.wood
    }
    if (name.includes('약초') || name.includes('herb')) {
      return ITEM_ICONS.herb
    }
    return ITEM_ICONS.ore
  }
