...
export class AchievementService {
  ...
  private matchesCondition(
    ...
    // 대상 매칭 (있는 경우)
    if (condition.target && condition.target !== value) return false
  ...
  private async grantRewards(userId: string, rewards: Achievement['rewards']): Promise<void> {
    try {
      ...
      if (rewards.gold) {
        // TODO: 골드 지급 구현 필요
        console.log(`⚠️ Gold rewards not implemented yet:`, rewards.gold)
      }

      if (rewards.items) {
        // TODO: 아이템 지급 구현 필요
        console.log(`⚠️ Item rewards not implemented yet:`, rewards.items)
      }

      if (rewards.energy) {
        // TODO: 에너지 지급 구현 필요
        console.log(`⚠️ Energy rewards not implemented yet:`, rewards.energy)
      }

      if (rewards.stat) {
        // 스탯 보너스 지급 - 기본 4개 스탯만 처리
        const basicStats: ('health' | 'learning' | 'relationship' | 'achievement')[] = 
          ['health', 'learning', 'relationship', 'achievement']
        
        if (basicStats.includes(rewards.stat.type as typeof basicStats[number])) {
          await dbHelpers.addActivity({
            userId,
            statType: rewards.stat.type as 'health' | 'learning' | 'relationship' | 'achievement',
            activityName: '업적 스탯 보너스',
            description: '업적 달성 보너스',
            experience: rewards.stat.value,
            timestamp: new Date(),
            synced: false
          })
        } else {
          // 게임 스탯(attack, defense 등)은 별도 처리 필요
          console.log(`⚠️ Game stat rewards not implemented yet:`, rewards.stat)
        }
      }

      // 칭호, 특별 보상 등은 별도 처리
      if (rewards.title) {
        console.log(`Granting title "${rewards.title}" to user ${userId}`)
      }

      if (rewards.specialReward) {
        console.log(`Granting special reward to user ${userId}:`, rewards.specialReward)
      }
    } catch (error) {
